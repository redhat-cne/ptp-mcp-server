    ...
    def _extract_per_component_offsets(self, logs) -> Dict[str, Any]:
        ...
        for component in ("ptp4l", "phc2sys", "ts2phc"):
            ...
            if comp_logs:
                ...
                offsets[component] = latest.parsed_data["offset"]

        return offsets

    def _extract_per_component_servo_states(self, logs) -> Dict[str, Any]:
        """Extract the most recent servo state from each PTP component."""
        servo_states = {}

        for component in ("dpll", "ptp4l", "phc2sys", "ts2phc"):
            comp_logs = [
                log for log in logs
                if component in log.component.lower() and log.parsed_data.get("state")
            ]
            if comp_logs:
                latest = max(comp_logs, key=lambda x: x.timestamp)
                servo_states[component] = latest.parsed_data.get("state")
                ...
                if check_sync:
                    try:
                        ...
                        sync_check = {
                            ...
                            or self._extract_per_component_servo_states(logs),
                        }
