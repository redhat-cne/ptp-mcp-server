    ...
    def extract_grandmaster_info(self, logs: List[LogEntry]) -> Dict[str, Any]:
        ...
        for component in ("ptp4l", "phc2sys", "ts2phc"):
            ...
            if comp_logs:
                ...
                gm_info["last_offsets"][component] = latest.parsed_data["offset"]
                if latest.parsed_data.get("frequency") is not None:
                    gm_info["last_frequencies"][component] = latest.parsed_data["frequency"]
    ...
    def extract_sync_status(self, logs: List[LogEntry]) -> Dict[str, Any]:
        ...
        if dpll_logs:
            ...
            if dpll_component_logs:
                ...
                if comp_parsed.get("state"):
                    sync_status["last_servo_states"]["dpll"] = comp_parsed["state"]
            ...
            if parsed.get("dpll_state"):
                ...
                sync_status["last_servo_states"]["dpll"] = parsed["dpll_state"]
        ...
        if phc2sys_logs:
            ...
            if servo_state:
                sync_status["last_servo_states"]["phc2sys"] = servo_state
        ...
        if ptp4l_offset_logs:
            ...
            sync_status["last_offsets"]["ptp4l"] = latest_ptp4l_offset.parsed_data["offset"]
            ptp4l_state = latest_ptp4l_offset.parsed_data.get("state")
            if ptp4l_state:
                sync_status["last_servo_states"]["ptp4l"] = ptp4l_state
        ...
        if ts2phc_offset_logs:
            ...
            sync_status["last_offsets"]["ts2phc"] = latest_ts2phc_offset.parsed_data["offset"]
            ts2phc_state = latest_ts2phc_offset.parsed_data.get("state")
            if ts2phc_state:
                sync_status["last_servo_states"]["ts2phc"] = ts2phc_state
